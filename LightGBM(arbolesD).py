...
def extract_basic_features(texts):
    ...
    features['count_punct'] = texts.apply(lambda x: sum(1 for c in x if c in string.punctuation))
    return features
...
# Vectorizador TF-IDF
vectorizer = TfidfVectorizer(stop_words=spanish_stopwords, max_features=500, ngram_range=(1, 2))
